...
export default function VolunteerPerformanceDashboard() {
  const { data: performanceData, isLoading } =
    trpc.volunteers.getAllPerformance.useQuery();
  ...
  const totalDeliveries = performanceData.reduce(
    (sum, v) => sum + (v.totalDeliveries || 0),
    0
  );
  const avgDeliveryTime = Math.round(
    performanceData.reduce((sum, v) => sum + (v.avgDeliveryTime || 0), 0) /
      totalVolunteers
  );
  const avgOnTimeRate = Math.round(
    performanceData.reduce((sum, v) => sum + (v.onTimeRate || 0), 0) /
      totalVolunteers
  );
  ...
  const onTimeRateDistribution = [
    ...
    },
    {
      ...
      value: performanceData.filter(
        v => (v.onTimeRate || 0) >= 70 && (v.onTimeRate || 0) < 90
      ).length,
      color: "#eab308",
    },
    {
      name: "50-70%",
      value: performanceData.filter(
        v => (v.onTimeRate || 0) >= 50 && (v.onTimeRate || 0) < 70
      ).length,
      color: "#f97316",
    },
    {
  ...
  return (
    <div className="container mx-auto py-8 space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">追蹤志工送餐表現，優化配送效率</p>
      </div>
                  ...
                  .map(volunteer => (
                    ...
                    >
                      <td className="p-2 font-medium">
                        {volunteer.name || `志工${volunteer.id}`}
                      </td>
                      <td className="text-right p-2">
                        {volunteer.totalDeliveries || 0}
                      </td>
                      <td className="text-right p-2">
                        {volunteer.completedDeliveries || 0}
                      </td>
                      <td className="text-right p-2">
                        {volunteer.avgDeliveryTime || 0} 分鐘
                      </td>
                      <td className="text-right p-2">
                        {volunteer.onTimeRate || 0}%
                      </td>
