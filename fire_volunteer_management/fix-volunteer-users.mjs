...
async function fixVolunteerUsers() {
  ...
  const eddieUser = await db.select().from(users).where(eq(users.name, "EDDIE LIN")).limit(1);
  ...
  const volunteersWithEddie = await db.select().from(volunteers).where(eq(volunteers.userId, eddieUserId));
  ...
  for (const volunteer of volunteersWithEddie) {
    ...
    console.log(`為志工 ${volunteer.employeeId || volunteer.id} 建立使用者帳號...`);
    ...
    console.log(`✓ 志工 ${volunteer.employeeId || volunteer.id} 已關聯到新使用者 (ID: ${newUser.insertId})`);
  }
  
  console.log("清理完成！");
}

fixVolunteerUsers().catch(console.error).finally(() => process.exit(0));
