...
export async function sendPublicBookingConfirmationEmail(
  ...
  text += `預約資訊：\n`;
  text += `查詢編號：${bookingNumber}\n`;
  text += `參訪日期：${visitDate}\n`;
  text += `參訪時段：${visitTime}\n`;
  text += `參訪人數：${visitorCount}人\n\n`;
  text += `請於參訪當日提早10分鐘抵達，並出示此查詢編號。\n\n`;
  text += `如需取消或變更預約，請至少於參訪日前3天聯繫我們。\n\n`;
  text += `聯繫資訊：\n`;
  text += `地址：台東市更生北路616巷9號\n`;
  text += `電話：(089)334547、348138\n`;
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background-color: #f0fdf4; padding: 20px; border-left: 4px solid #16a34a; margin: 20px 0;">
        ...
        <p style="margin: 5px 0;"><strong>查詢編號：</strong> ${bookingNumber}</p>
      ...
      <div style="background-color: #fef3c7; padding: 15px; border-left: 4px solid #f59e0b; margin: 20px 0;">
        ...
        <ul style="margin: 10px 0; padding-left: 20px;">
          ...
          <li>請出示此查詢編號：<strong>${bookingNumber}</strong></li>
      ...
      <div style="background-color: #f3f4f6; padding: 15px; margin: 20px 0;">
        ...
        <p style="margin: 5px 0;">電話：(089)334547、348138</p>
...
export async function sendBookingCancellationEmail(
  ...
  text += `取消預約資訊：\n`;
  text += `查詢編號：${bookingNumber}\n`;
  text += `原訂參訪日期：${visitDate}\n`;
  text += `原訂參訪時段：${visitTime}\n\n`;
  text += `如需重新預約，歡迎隨時透過我們的線上預約系統進行預約。\n\n`;
  text += `聯繫資訊：\n`;
  text += `地址：台東市更生北路616巷9號\n`;
  text += `電話：(089)334547、348138\n`;
  text += `Email: info@taitung-disaster.gov.tw\n\n`;
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background-color: #fef2f2; padding: 20px; border-left: 4px solid #dc2626; margin: 20px 0;">
        <h3 style="margin-top: 0; color: #dc2626;">取消預約資訊</h3>
        <p style="margin: 5px 0;"><strong>查詢編號：</strong> ${bookingNumber}</p>
        <p style="margin: 5px 0;"><strong>原訂參訪日期：</strong> ${visitDate}</p>
        <p style="margin: 5px 0;"><strong>原訂參訪時段：</strong> ${visitTime}</p>
      </div>
      <p>如需重新預約，歡迎隨時透過我們的線上預約系統進行預約。</p>
      <div style="background-color: #f3f4f6; padding: 15px; margin: 20px 0;">
        <h4 style="margin-top: 0;">聯繫資訊</h4>
        <p style="margin: 5px 0;">地址：台東市更生北路616巷9號</p>
        <p style="margin: 5px 0;">電話：(089)334547、348138</p>
