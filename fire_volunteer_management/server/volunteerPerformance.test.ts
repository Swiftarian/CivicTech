...
describe("志工績效統計API", () => {
  it("管理員可以查詢所有志工績效統計", async () => {
    ...
    if (result.length > 0) {
      ...
      expect(firstVolunteer.completedDeliveries).toBeLessThanOrEqual(
        firstVolunteer.totalDeliveries
      );
  ...
  it("績效統計數據應該合理", async () => {
    ...
    for (const volunteer of result) {
      // 完成次數不應超過總次數
      expect(volunteer.completedDeliveries).toBeLessThanOrEqual(
  ...
  it("應該能夠按送餐次數排序志工", async () => {
    ...
    if (result.length > 1) {
      ...
      const sorted = [...result].sort(
        (a, b) => (b.totalDeliveries || 0) - (a.totalDeliveries || 0)
      );

      // 驗證排序後的第一個志工送餐次數最多
      expect(sorted[0].totalDeliveries).toBeGreaterThanOrEqual(
        sorted[sorted.length - 1].totalDeliveries || 0
      );
    }
  });
});
