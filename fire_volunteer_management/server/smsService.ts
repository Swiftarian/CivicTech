...
`.trim();
  ...
  try {
    ...
    if (twilioClient && process.env.TWILIO_FROM_NUMBER) {
      ...
      console.log("=".repeat(60));
      console.log(
        `收件人：${recipientName} (${recipientPhone} -> ${formattedPhone})`
      );
      ...
      console.log("=".repeat(60));
      console.log(
        "⚠️  模擬模式：請設定TWILIO_ACCOUNT_SID、TWILIO_AUTH_TOKEN、TWILIO_FROM_NUMBER環境變數以啟用真實SMS發送"
      );
      console.log("=".repeat(60));
...
export async function sendDeliveryReminderSMS(params: {
  ...
  const { recipientPhone, recipientName, verificationCode, estimatedTime } =
    params;
