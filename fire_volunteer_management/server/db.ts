import { eq, and, desc, gte, lte, or, like, sql, isNotNull, inArray } from "drizzle-orm";
import { drizzle } from "drizzle-orm/mysql2";
import { 
  InsertUser, users, 
  volunteers, InsertVolunteer,
  bookings, InsertBooking,
  individualBookings, InsertIndividualBooking,
  groupBookings, InsertGroupBooking,
  ...
  deliveryTasks, InsertDeliveryTask,
  deliveryPoints, InsertDeliveryPoint,
  notifications, InsertNotification,
  emailLogs, InsertEmailLog, type EmailLog,
  news, InsertNews, type News,
  gallery, InsertGallery, type Gallery,
  homeContent, InsertHomeContent, type HomeContent
} from "../drizzle/schema";
...
export async function updateHomeContent(data: Partial<InsertHomeContent> & { id: number }) {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot update home content: database not available");
    return;
  }
...
export async function getVolunteersByUserId(userId: number) {
  const db = await getDb();
  if (!db) return [];
  const result = await db.select().from(volunteers).where(eq(volunteers.userId, userId));
  return result;
}
...
export async function getBookingByNumber(bookingNumber: string) {
  ...
  const individualResult = await db.select().from(individualBookings)
    .where(eq(individualBookings.bookingNumber, bookingNumber)).limit(1);
  ...
  const groupResult = await db.select().from(groupBookings)
    .where(eq(groupBookings.bookingNumber, bookingNumber)).limit(1);
...
export async function getAllBookings() {
  ...
  // 查詢個人預約
  const individualResults = await db.select().from(individualBookings).orderBy(desc(individualBookings.createdAt));
  const individualWithType = individualResults.map(b => ({ ...b, type: 'individual' as const }));
  
  // 查詢團體預約
  const groupResults = await db.select().from(groupBookings).orderBy(desc(groupBookings.createdAt));
  const groupWithType = groupResults.map(b => ({ ...b, type: 'group' as const }));
...
// 編輯預約資料
export async function updateBooking(id: number, type: 'individual' | 'group', data: {
  contactName?: string;
  contactPhone?: string;
  contactEmail?: string;
  ...
  if (type === 'individual') {
    await db.update(individualBookings)
      .set(data)
      .where(eq(individualBookings.id, id));
  } else {
    await db.update(groupBookings)
      .set(data)
...
export async function getIndividualBookingsByDateRange(startDate: Date, endDate: Date) {
  const db = await getDb();
  if (!db) return [];
  
  const results = await db.select().from(individualBookings)
    .where(and(gte(individualBookings.visitDate, startDate), lte(individualBookings.visitDate, endDate)))
...
export async function getGroupBookingsByDateRange(startDate: Date, endDate: Date) {
  const db = await getDb();
  if (!db) return [];
  
  const results = await db.select().from(groupBookings)
    .where(and(gte(groupBookings.visitDate, startDate), lte(groupBookings.visitDate, endDate)))
...
export async function getBookingsByDateRange(startDate: Date, endDate: Date) {
  ...
  // 查詢個人預約
  const individualResults = await db.select().from(individualBookings)
    .where(and(gte(individualBookings.visitDate, startDate), lte(individualBookings.visitDate, endDate)))
    .orderBy(individualBookings.visitDate);
  
  // 查詢團體預約
  const groupResults = await db.select().from(groupBookings)
    .where(and(gte(groupBookings.visitDate, startDate), lte(groupBookings.visitDate, endDate)))
    .orderBy(groupBookings.visitDate);
...
export async function updateBookingStatus(id: number, status: "pending" | "confirmed" | "cancelled" | "completed", bookingType?: 'individual' | 'group') {
  const db = await getDb();
  if (!db) return;
  
  // 根據預約類型更新對應的表
  if (bookingType === 'individual') {
    await db.update(individualBookings).set({ status }).where(eq(individualBookings.id, id));
...
export async function getAllDeliveryTasks(filters?: {
  ...
  let query = db.select().from(deliveryTasks).orderBy(desc(deliveryTasks.taskDate));
 ...
 * 指派送餐任務給義工
 */
export async function assignDeliveryTask(taskId: number, volunteerId: number, volunteerName: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getVolunteerDeliveryStats() {
  ...
  // 查詢所有志工及其使用者資料
  const volunteersData = await db.select({
    volunteerId: volunteers.id,
    employeeId: volunteers.employeeId,
    department: volunteers.department,
    position: volunteers.position,
...
export async function deleteMealDelivery(id: number): Promise<void> {
  ...
  if (!database) {
    console.warn("[Database] Cannot delete meal delivery: database not available");
    return;
  }
...
export async function batchDeleteMealDeliveries(ids: number[]): Promise<void> {
  const database = await getDb();
  if (!database) {
    console.warn("[Database] Cannot batch delete meal deliveries: database not available");
    return;
  }

  try {
    // 使用 Drizzle ORM 的 inArray 函數批次刪除
    await database.delete(mealDeliveries).where(inArray(mealDeliveries.id, ids));
  } catch (error) {
...
export async function createNews(newsData: InsertNews): Promise<News> {
  ...
  try {
    ...
    const inserted = await database.select().from(news).where(eq(news.id, insertedId)).limit(1);
    return inserted[0]!;
  } catch (error) {
...
export async function updateNews(id: number, newsData: Partial<InsertNews>): Promise<void> {
  const database = await getDb();
  if (!database) {
    throw new Error("Database not available");
  }
...
export async function getNewsById(id: number): Promise<News | undefined> {
  ...
  try {
    const result = await database.select().from(news).where(eq(news.id, id)).limit(1);
    return result[0];
  } catch (error) {
...
export async function createGalleryItem(galleryData: InsertGallery): Promise<Gallery> {
  const database = await getDb();
  if (!database) {
    throw new Error("Database not available");
  }
  ...
  try {
    ...
    const insertedId = Number(result[0].insertId);
    const inserted = await database.select().from(gallery).where(eq(gallery.id, insertedId)).limit(1);
    return inserted[0]!;
  } catch (error) {
...
export async function updateGalleryItem(id: number, galleryData: Partial<InsertGallery>): Promise<void> {
  const database = await getDb();
  if (!database) {
    throw new Error("Database not available");
  }
...
export async function getGalleryById(id: number): Promise<Gallery | undefined> {
  ...
  try {
    const result = await database.select().from(gallery).where(eq(gallery.id, id)).limit(1);
    return result[0];
  } catch (error) {
