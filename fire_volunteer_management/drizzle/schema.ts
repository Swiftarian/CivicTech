...
export const volunteers = mysqlTable("volunteers", {
  ...
  category: mysqlEnum("category", ["導覽館志工", "送餐志工"]).default("導覽館志工").notNull(), // 志工分類
...
export const recipients = mysqlTable("recipients", {
  ...
  // 通知偏好
  preferredNotificationMethod: mysqlEnum("preferredNotificationMethod", ["line", "sms", "both"]).default("sms"),
...
export const deliveryPoints = mysqlTable("deliveryPoints", {
  id: int("id").autoincrement().primaryKey(),
  taskId: int("taskId").notNull().references(() => deliveryTasks.id), // 關聯到送餐任務
...
export const news = mysqlTable("news", {
  ...
  category: mysqlEnum("category", ["防災宣導", "活動公告", "新聞稿", "其他"]).default("其他").notNull(), // 分類
...
export const gallery = mysqlTable("gallery", {
  ...
  category: mysqlEnum("category", ["活動花絮", "設施環境", "教育訓練", "志工服務", "其他"]).default("其他").notNull(), // 分類
...
export const homeContent = mysqlTable("homeContent", {
  id: int("id").autoincrement().primaryKey(),
  // 關於教育館的介紹文字
  aboutTitle: varchar("aboutTitle", { length: 200 }).default("關於臺東災害警覺教育館").notNull(),
