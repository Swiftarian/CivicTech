        ...
        if email_input:
            ...
            if cases:
                ...
                if selected['selection']['rows']:
                    ...
                    status_color = ":red"
                    status_icon = "⏳"
                    if selected_case['status'] == "審核中":
                        status_color = ":orange"
                        status_icon = "🔄"
                    elif selected_case['status'] == "可領件":
                        status_color = ":green"
                        status_icon = "✅"
                    ...
                    with col2:
                        ...
                        **目前狀態**: {status_icon} {status_color}[{selected_case['status']}]  
                        """)
                    ...
                    if selected_case['status'] == "可領件":
                        st.success("🎉 恭喜！您的案件已審核通過，請攜帶身分證件至本局領取核定書表。")
                    elif selected_case['status'] == "已退件":
                        st.error("⚠️ 您的案件已被退件，請依審核備註說明修正後重新送件。")
                    elif selected_case['status'] == "待補件":
                        st.warning("📝 您的案件需要補件，請儘速補齊相關文件。")
